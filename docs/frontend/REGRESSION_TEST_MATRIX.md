...
| R05 | Dataset | `/projects/{projectId}/datasets/{datasetId}` | Case 상태 전이, Version 관리, 404 복귀, Project Scope 대조 | 유효 Project와 Dataset | DRAFT, APPROVED Case | Case와 Dataset Version 표시, 존재하지 않거나 다른 Project 소속인 Dataset은 복귀 Action 표시 | STATIC_ONLY | Typecheck, Lint, Production Build | - | Repository Evidence | Browser E2E | 404 복귀 Action 코드 구현 완료. Frontend URL Project Scope 대조 구현 완료(Scope 불일치 시 Resource 미노출, 기존 DATASET_NOT_FOUND UX 재사용, Retry 미노출). Browser 검증 전이며 VERIFIED_MANUAL 아님. Backend Authorization 또는 Scope 필터 구현이 아님 |
| R06 | Dataset Version | `/projects/{projectId}/datasets/{datasetId}/versions/{version}` | 불변 Snapshot 조회, 404 복귀, Project Scope 대조 | Dataset Version 존재 | Snapshot Case | Hash와 Case Snapshot 표시, 존재하지 않거나 다른 Project 소속인 Version은 복귀 Action 표시 | STATIC_ONLY | Typecheck, Lint, Production Build | - | Repository Evidence | Browser E2E | 404 복귀 Action과 잘못된 Version 형식 메시지 코드 구현 완료. Frontend URL Project Scope 대조 및 Version parent ID 방어적 대조 구현 완료(Scope 불일치 시 Resource 미노출, 기존 DATASET_VERSION_NOT_FOUND UX 재사용, Retry 미노출). Browser 검증 전이며 VERIFIED_MANUAL 아님. Dataset 없음과 Version 없음은 Backend가 동일 코드로 응답. Backend Authorization 또는 Scope 필터 구현이 아님 |
| R07 | Target | `/projects/{projectId}/targets/{targetId}` | MOCK 설정과 FIXED Version 관리, Project Scope 대조 | 유효 Target | Target Version 1, 2 | 설정과 불변 Version 표시, 다른 Project 소속인 Target은 복귀 Action 표시 | STATIC_ONLY | Typecheck, Lint, Production Build | - | Repository Evidence | Browser E2E | Frontend URL Project Scope 대조 구현 완료(Scope 불일치 시 Resource 미노출, 기존 TARGET_NOT_FOUND UX 재사용, Retry 미노출). Browser 검증 전이며 VERIFIED_MANUAL 아님. Backend Authorization 또는 Scope 필터 구현이 아님 |
| R08 | Target Version | `/projects/{projectId}/targets/{targetId}/versions/{version}` | FIXED Snapshot 조회, 404 복귀, Project Scope 대조 | Target Version 또는 404 | Target Version 1, 2 | Snapshot 또는 Target 복귀 Action 표시, 다른 Project 소속인 Version은 복귀 Action 표시 | STATIC_ONLY | Typecheck, Lint, Production Build | - | Repository Evidence | Browser E2E | Frontend URL Project Scope 대조 및 Version parent ID 방어적 대조 구현 완료(Scope 불일치 시 Resource 미노출, 기존 TARGET_VERSION_NOT_FOUND UX 재사용, Retry 미노출). Browser 검증 전이며 VERIFIED_MANUAL 아님. Backend Authorization 또는 Scope 필터 구현이 아님. Target/Evaluator Version API 요청 AbortSignal 보강은 별도 작업(v0.25.3) |
| R09 | Evaluator | `/projects/{projectId}/evaluators/{evaluatorId}` | 설정과 Version 관리, Project Scope 대조 | 유효 Evaluator | 세 Evaluator Type | 설정과 불변 Version 표시, 다른 Project 소속인 Evaluator는 복귀 Action 표시 | STATIC_ONLY | Typecheck, Lint, Production Build | - | Repository Evidence | Browser E2E | Frontend URL Project Scope 대조 구현 완료(Scope 불일치 시 Resource 미노출, 기존 EVALUATOR_NOT_FOUND UX 재사용, Retry 미노출). Browser 검증 전이며 VERIFIED_MANUAL 아님. Backend Authorization 또는 Scope 필터 구현이 아님 |
| R10 | Evaluator Version | `/projects/{projectId}/evaluators/{evaluatorId}/versions/{version}` | Evaluator Snapshot 조회, 404 복귀, Project Scope 대조 | Evaluator Version 또는 404 | Version 1, 2와 REGEX | Snapshot 또는 Evaluator 복귀 Action 표시, 다른 Project 소속인 Version은 복귀 Action 표시 | STATIC_ONLY | Typecheck, Lint, Production Build | - | Repository Evidence | Browser E2E | Frontend URL Project Scope 대조 및 Version parent ID 방어적 대조 구현 완료(Scope 불일치 시 Resource 미노출, 기존 EVALUATOR_VERSION_NOT_FOUND UX 재사용, Retry 미노출). Browser 검증 전이며 VERIFIED_MANUAL 아님. Backend Authorization 또는 Scope 필터 구현이 아님. Target/Evaluator Version API 요청 AbortSignal 보강은 별도 작업(v0.25.3) |
