...
| R05 | Dataset | `/projects/{projectId}/datasets/{datasetId}` | Case 상태 전이, Version 관리, 404 복귀 | 유효 Project와 Dataset | DRAFT, APPROVED Case | Case와 Dataset Version 표시, 존재하지 않는 Dataset은 복귀 Action 표시 | STATIC_ONLY | Typecheck, Lint, Production Build | - | Repository Evidence | Browser E2E | 404 복귀 Action(Project Overview·Project 목록) 코드 구현 완료. Browser 검증 전이며 VERIFIED_MANUAL 아님. Project Scope 대조는 별도 작업이며 이번 PR 범위 아님 |
| R06 | Dataset Version | `/projects/{projectId}/datasets/{datasetId}/versions/{version}` | 불변 Snapshot 조회, 404 복귀 | Dataset Version 존재 | Snapshot Case | Hash와 Case Snapshot 표시, 존재하지 않는 Version은 복귀 Action 표시 | STATIC_ONLY | Typecheck, Lint, Production Build | - | Repository Evidence | Browser E2E | 404 복귀 Action(Dataset 상세·Project Overview)과 잘못된 Version 형식 메시지 코드 구현 완료. Browser 검증 전이며 VERIFIED_MANUAL 아님. Dataset 없음과 Version 없음은 Backend가 동일 코드로 응답. Project Scope 대조는 별도 작업이며 이번 PR 범위 아님 |
