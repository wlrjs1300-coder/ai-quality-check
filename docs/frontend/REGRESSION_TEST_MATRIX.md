...
| R10 | Evaluator Version | `/projects/{projectId}/evaluators/{evaluatorId}/versions/{version}` | Evaluator Snapshot 조회, 404 복귀, Project Scope 대조, AbortSignal 보강 | Evaluator Version 또는 404 | Version 1, 2와 REGEX | Snapshot 또는 Evaluator 복귀 Action 표시, 다른 Project 소속인 Version은 복귀 Action 표시 | STATIC_ONLY | Typecheck, Lint, Production Build | - | Repository Evidence | Browser E2E | Frontend URL Project Scope 대조 및 Version parent ID 방어적 대조 구현 완료(Scope 불일치 시 Resource 미노출, 기존 EVALUATOR_VERSION_NOT_FOUND UX 재사용, Retry 미노출). Evaluator Version API 요청에 동일 AbortSignal 전달, 이전 요청 Abort, requestId+controller 결합 최신성 검사, AbortError UI 미노출, unmount cleanup 구현 완료. Browser 검증 전이며 VERIFIED_MANUAL 아님. Backend Authorization 또는 Scope 필터 구현이 아님. Invalid Version 처리는 이번 작업 범위 아님 |
